...
class MarriageAllowanceDESConnector @Inject() (
  ...
  private def evaluateCodes(
    findRecipientResponseDES: FindRecipientResponseDES
  ): Either[DataRetrievalError, UserRecord] =
    (findRecipientResponseDES.returnCode, findRecipientResponseDES.reasonCode) match {
      ...
      case codes @ (ErrorReturnCode, NinoNotFound)               =>
        generateResponse(
          FindRecipientCodedErrorResponse(codes._1, codes._2, "Nino not found and Nino not found in merge trail")
        )
      case codes @ (ErrorReturnCode, MultipleNinosInMergeTrail)  =>
        generateResponse(
          FindRecipientCodedErrorResponse(codes._1, codes._2, "Nino not found and Nino found in multiple merge trails")
        )
      ...
      case codes @ (ErrorReturnCode, OnlyOneNinoOrTempReference) =>
        generateResponse(
          FindRecipientCodedErrorResponse(
            codes._1,
            codes._2,
            "Only one of Nino or Temporary Reference must be supplied"
          )
        )
  ...
  def sendMultiYearCreateRelationshipRequest(
    ...
    val path =
      url"$serviceUrl/marriage-allowance/02.00.00/citizen/${createRelationshipRequest.recipientCid}/relationship/$relType"
    http
      .post(path)
  ...
  def updateAllowanceRelationship(
    ...
    val path =
      url"$serviceUrl/marriage-allowance/citizen/${updateRelationshipRequest.participant1.instanceIdentifier}/relationship"
    http
      .put(path)
